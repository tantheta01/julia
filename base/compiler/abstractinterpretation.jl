...
function abstract_call_gf_by_type(interp::AbstractInterpreter, @nospecialize(f), argtypes::Vector{Any}, @nospecialize(atype), sv::InferenceState,
    ...
    #print("=> ", rettype, "\n")
    rettype isa LimitedAccuracy && (union!(sv.pclimitations, rettype.causes); rettype = rettype.typ)
    isempty(sv.pclimitations) || delete!(sv.pclimitations, sv) # remove self, if present
...
function abstract_eval_value(interp::AbstractInterpreter, @nospecialize(e), vtypes::VarTable, sv::InferenceState)
    if isa(e, Expr)
        ...
        typ isa LimitedAccuracy && (union!(sv.pclimitations, typ.causes); typ = typ.typ)
        return typ
    end
end
